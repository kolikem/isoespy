...
# for evaluating concordance of NESs in ValidNES proteins

my $NES_profile = "$Bin/NES_profiles_trained.txt";
my $threshold_score = 2;
my $cut_off = -10;
my $notout_predicted_NES = 0;
my $output_predicted_NES_except_true = 0;
my $help;
...
GetOptions(
    ...
    'outneg|on' => \$output_predicted_NES_except_true,
    'help|h' => \$help
...
=head1 SYNOPSIS
  ...
  Options:
   ...
   --outneg or -on      output predicted NESs other than true NES regions  [default: false]
...
my $max_hydrophobicity_rate = 1;      # allowable rate of hydrophobic residues in NES spacer region
    ...
    while (my $line = <FILE>){
        ...
        if ($line =~ /^>(.+)/){
            if ($Seq ne ''){
                ...
                my ($name, $range, $protein) = split (/\s+/, $header);
                if (!exists $nes_range_protein{$protein}){
                    $nes_range_protein{$protein} = $range;
                }
                else{
                    $nes_range_protein{$protein} .= '==' . $range;
                }
...
#print STDERR "Number of proteins in ValidNES: ", scalar keys %protein_id_2, "\n";
...
foreach my $name (sort keys %input_seq){
    ...
    my $count = 0;
    my ($id, $range, $pro_id) = split (/\s+/, $name);
    $check_duplicate{$pro_id} ++;
    $total_protein ++ if ($check_duplicate{$pro_id} <= 1);
    while (@sum_array > 4){
        ...
        if ($score >= $threshold_score){
            ...
            if (($pos >= $start - 14) and ($pos <= $end)){
                $hit_query{$name} = 1;
                $overlap_flag = 1;
                if ($notout_predicted_NES == 0){
                    $true_nes{$id} = "$pos\t$nes\t$score";
                }
            }
            if ($nes_range_protein{$pro_id} =~ /==/){
                my @range = split (/==/, $nes_range_protein{$pro_id});
                foreach my $range2 (@range){
                    if ($range ne $range2){
                        my ($start2, $end2) = split (/-/, $range2);
                        if (($pos >= $start2 - 14) and ($pos <= $end2)){
                            $overlap_flag = 1;
                        }
                    }
                }
            }
            if (($overlap_flag == 0) and ($check_duplicate{$pro_id} <= 1)){
                $hit_nes_outside{$name_pos} = 1;
                for (my $i = 0; $i < 14; $i++){
                    my $pos_2 = $pos + $i;
                    $hit_pos{$pos_2} = 1;
                }
                if ($output_predicted_NES_except_true == 1){
                    ${$false_nes{$pro_id}}{$pos} = "$pos\t$nes\t$score";
                }
            }
        }
    }
...
my $hit_nes_num = scalar keys %hit_nes_outside;
my $rate_hit = int ($hit_query_num / $total_query * 1000) / 10;

#print STDERR "Total protein: $total_protein\n";

if ($notout_predicted_NES == 0){
    foreach my $id (sort keys %true_nes){
        print "$id\t$true_nes{$id}\n";
    }
}

if ($output_predicted_NES_except_true == 1){
    foreach my $id (sort keys %false_nes){
        foreach my $pos (sort {$a <=> $b} keys %{$false_nes{$id}}){
            print "$id\t${$false_nes{$id}}{$pos}\n";
        }
    }
}

print "Query number: $total_query\n";
print "Number of true positives (score >= $threshold_score): $hit_query_num (TPR: $rate_hit%)\n";
print "Number of false positives (score >= $threshold_score): $hit_nes_num\n";
